//Declaration of variables and defintion of the canvas.   
var w = window.innerWidth,
      ...
      // We're passing in a function in d3.max to tell it what we're maxing (x value), in this case Year variable.
      var xScale = d3.scale.linear()                
          .domain([1997, d3.max(malaga, function (d) { return d.Year; })])
          .range([margin.left, w - margin.right]);  // Set margins for x specific

      // We're passing in a function in d3.max to tell it what we're maxing (y value), in this case Value variable.
      ...
      var yAxis = d3.svg.axis().scale(yScale).orient("left").tickFormat(d3.format(''));
       ...
       // Adds X-Axis as a 'g' element, as well as the label of the axis
      svg.append("g").attr({
      ...
      // Adds Y-Axis as a 'g' element, as well as the label of the axis.
      svg.append("g").attr({
        "class": "axis",
    ...
    // Definition of the circles with position according to our data values.
    var circleAttrs = {           
     ...
     // Select that all circles have the attributes of interactivity (MouseOver,MouseOut, on "click").
